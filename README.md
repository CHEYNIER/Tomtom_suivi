...
<head>
    ...
    <!-- Importation des bibliothèques nécessaires pour React, ReactDOM, Tailwind et Recharts -->
    ...
    <style>
        /* Styles CSS pour améliorer l'expérience PWA */
        body { margin: 0; padding: 0; background-color: #f8f9fa; }
        /* Pour s'assurer que l'app utilise tout l'écran */
...
<body>
    ...
    <script type="text/babel">
        // Ceci est le contenu de votre ancien fichier TomTomActivityTracker.jsx, adapté
        ...
        // Les variables globales du Canvas (__app_id, __firebase_config, __initial_auth_token) sont ici simulées car Vercel est un environnement externe.
        // NOTE: Si vous exécutez ceci dans l'Immersive, les vraies variables seront disponibles.
        const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {};
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'tomtom-tracker-pwa-001'; 
        const initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;

        // Initialisation des instances Firebase (avec vérification)
        ...
        const App = () => {
          ...
          useEffect(() => {
            if (!auth || !db) {
                setError("La configuration Firebase est manquante. L'application ne peut pas stocker les données. Assurez-vous d'avoir configuré Firebase pour ce projet.");
                setLoading(false);
                return;
            }
            ...
            const handleAuth = async () => {
                try {
                    ...
                    setError("Échec de la connexion. Vérifiez la configuration Firebase.");
                }
            };
          ...
          // 2. Récupération des données d'activité en temps réel
          useEffect(() => {
            if (!isAuthReady || !userId || !db) return;

            // Chemin Firestore pour les données privées de l'utilisateur
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
              ...
              // Tri manuel par date la plus récente
              fetchedActivities.sort((a, b) => b.date.toDate().getTime() - a.date.toDate().getTime()); 
              setActivities(fetchedActivities);
            }, (e) => {
                console.error("Erreur de récupération Firestore:", e);
                setError("Impossible de charger les activités. Problème de connexion ou de permissions.");
            });
          ...
          const addActivity = async (data) => {
            if (!userId || !db) {
                alert("Veuillez vous authentifier pour ajouter des activités.");
                return;
            }
            setLoading(true);
            const activityData = {
            ...
            try {
                ...
                // Utilisation d'une boîte de message personnalisée au lieu d'alert()
                showMessage("Activité enregistrée avec succès !", "success");
            } catch (e) {
                console.error("Erreur lors de l'ajout de l'activité:", e);
                showMessage("Échec de l'enregistrement de l'activité.", "error");
            } finally {
                setLoading(false);
            }
          };
          ...
          const showMessage = (message, type = 'info') => {
                // Vous pouvez implémenter un composant modal ou toast ici pour un meilleur UI
                console.log(`MESSAGE (${type.toUpperCase()}): ${message}`); 
                alert(message); // On utilise alert en dernier recours si pas de composant UI
          }


          // Préparation des données pour le graphique (distance quotidienne)
          const getChartData = () => {
            const dailyData = {};
            ...
            // On prend les 7 dernières dates, puis on les met dans l'ordre chronologique
          ...
          if (loading && !userId && !error) {
            return (
                <div className="flex items-center justify-center min-h-screen bg-gray-100">
                    <p className="text-xl text-gray-700">Connexion à la base de données...</p>
                </div>
            );
          }
              ...
              <div className="max-w-4xl mx-auto">
                <header className="text-center mb-8">
                    ...
                    <p className="text-gray-600 mt-1">
                        ...
                        <span className="font-mono text-xs text-red-500 break-all">{userId || 'Non Authentifié'}</span>
                    </p>
        ...
        // --- Composant Formulaire d'Activité ---
        const ActivityForm = ({ onSubmit, loading }) => {
            ...
            return (
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-lg mb-8 border-t-4 border-orange-500">
                    <h2 className="text-xl font-semibold mb-4 text-gray-700">Enregistrer une Nouvelle Activité (Manuelle)</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        ...
                        <button
                            ...
                            disabled={loading}
                        >
                            {loading ? 'Sauvegarde...' : 'Ajouter l\'Activité'}
                        </button>
                    </div>
                </form>
            );
        ...
        // --- Composant Liste des Activités ---
        const ActivityList = ({ activities }) => (
            <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-blue-500">
                <h2 className="text-xl font-semibold mb-4 text-gray-700">Historique des Activités ({activities.length})</h2>
                <ul className="space-y-3">
                    {activities.length === 0 ? (
                        <p className="text-gray-500 italic">Aucune activité enregistrée.</p>
                    ) : (
                        activities.map(activity => (
                            <li key={activity.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition duration-150 border-l-4 border-orange-400">
                                <div className="flex items-center">
                                    <span className="text-xl mr-3">{activity.type === 'Course' ? '🏃' : activity.type === 'Vélo' ? '🚴' : activity.type === 'Natation' ? '🏊' : '🚶'}</span>
                                    <div>
